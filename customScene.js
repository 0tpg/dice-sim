...
function deviceMotion(event) {
	let ax = event.acceleration ? event.acceleration.x : null;
	let ay = event.acceleration ? event.acceleration.y : null;
	let az = event.acceleration ? event.acceleration.z : null;

	if (ax == null && event.accelerationIncludingGravity) {
		let aig = event.accelerationIncludingGravity;

		gravX = GRAVITY_ALPHA * gravX + (1 - GRAVITY_ALPHA) * aig.x;
		gravY = GRAVITY_ALPHA * gravY + (1 - GRAVITY_ALPHA) * aig.y;
		gravZ = GRAVITY_ALPHA * gravZ + (1 - GRAVITY_ALPHA) * aig.z;
	...
	if (Math.abs(az) < 0.5) az = 0;

	accel_x = parseFloat(ax).toFixed(1);
	accel_y = parseFloat(ay).toFixed(1);
	accel_z = parseFloat(az).toFixed(1);
